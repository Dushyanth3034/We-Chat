...
const Login = () => {
  ...
  const handleGoogleLogin = () => {
    setError('');
    // Dynamically fallback to a default client ID if none is set in frontend environment variables
    const clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID || import.meta.env.VITE_GOOGLE_CLIENT_ID || 'google-client-id-placeholder';
  ...
  return (
    <div className="w-screen h-screen bg-gradient-to-br from-neutral-950 via-deepBg to-neutral-900 flex justify-center items-center px-4 overflow-auto">
      <div className="w-full max-w-md glass-panel rounded-3xl p-6 sm:p-8 shadow-2xl relative border border-neutral-800/80 animate-scale-up">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-white text-2xl font-bold tracking-tight">Welcome to WeChat</h2>
          <p className="text-neutral-500 text-sm mt-2">Sign in to stay connected with friends</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3.5 rounded-xl glass-input text-white text-sm bg-neutral-900/60"
              required
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-burgundy hover:bg-burgundy-light disabled:opacity-50 text-secondary font-semibold py-3.5 rounded-xl transition-all duration-300 shadow-lg shadow-burgundy/25 text-sm mt-2"
          >
          ...
          <button
            ...
            disabled={loading}
            className="w-full bg-white hover:bg-neutral-100 disabled:opacity-50 text-neutral-800 font-semibold py-3.5 rounded-xl transition-all duration-300 shadow-md text-sm flex items-center justify-center gap-3"
          >
        ...
        <p className="text-center text-neutral-500 text-sm mt-8">
          ...
          <Link to="/register" className="text-burgundy hover:text-burgundy-light font-medium transition-colors">
            Sign Up
          </Link>
        </p>
      </div>
